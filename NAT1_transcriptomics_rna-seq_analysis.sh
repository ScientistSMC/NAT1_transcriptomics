...
#SBATCH --ntasks 2 ##number of tasks (analyses) to run; default = 1
...
snakemake --cluster "sbatch -t {cluster.time} -p {cluster.partition} -c {cluster.threads} --mem {cluster.mem-per-cpu-mb}" --cluster-config cluster.slurm.cheaha.json --latency-wait=15 --use-conda --jobs 100
